...
class ShoreFollowerObserve {
    protected:
        ...
        unsigned long type_counter_[2]; // left, straight, right
    ...
    protected: // ROS Callbacks
        ...
        void image_callback(const sensor_msgs::ImageConstPtr & img_msg) {
            ...
            bool save_it = true;
            int label = -1;
            ROS_INFO("z: %f", (float)last_command_.angular.z);
            if ((last_command_.angular.z>0.1) && (last_command_.angular.z<1.0)) {
                label = 1;
            } else if ((last_command_.angular.z<-0.1) && (last_command_.angular.z>-1.0)) {
                label = 0;
            } else {
                return;
            }
            ROS_INFO("Label:  %d", (int)label);
            ...
            printf("Image counter at %ld (%ld %ld %ld)\n",image_counter_,
                    type_counter_[0],type_counter_[1]);
        }
    ...
    public:
        ShoreFollowerObserve() : nh_("~"), it_(nh_) {
            ...
            image_counter_ = 0;
            type_counter_[0] = type_counter_[1] = 0;
            ...
            joy_sub_ = nh_.subscribe("joy2",1,&ShoreFollowerObserve::joy_callback,this);
...
{
    ros::init(argc,argv,"shore_follower_observe2");
