...
class ShoreFollowerObserve {
    ...
    protected: // ROS Callbacks
        ...
        void image_callback(const sensor_msgs::ImageConstPtr & img_msg) {
            ...
            bool save_it = true;
            int label = -1;
            ROS_INFO("x: %f", (float)last_command_.linear.x);
            if ((last_command_.linear.x>0.2) && (last_command_.linear.x<0.3)) {
                label = 1;
            } else if ((last_command_.linear.x<-0.2) && (last_command_.linear.x>-0.3)) {
                label = 0;
            } else {
                return;
            }
